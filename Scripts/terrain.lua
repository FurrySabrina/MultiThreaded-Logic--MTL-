...
function Create(xMin, xMax, yMin, yMax, seed, data)
    ...
    local jWorld = sm.json.open("$CONTENT_DATA/Terrain/Worlds/World 32x32.world")
...
function GetColorAt(x, y, lod)
    ...
    return color.r + noise, color.g + noise, color.b + noise, 0
end

function GetMaterialAt(x, y, lod)
    local noise1 = sm.noise.octaveNoise2d(x / 8, y / 8, 4, g_cellData.seed*10)
    local normalizedNoise1 = (noise1+1) / 2
    local noise2 = sm.noise.octaveNoise2d(x / 8, y / 8, 4, g_cellData.seed*100)
